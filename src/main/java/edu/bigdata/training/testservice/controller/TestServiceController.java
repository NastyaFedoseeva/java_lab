...
public class TestServiceController {
    ...
    public ResponseEntity<PersonEntity> updatePerson(@PathVariable String id, @RequestBody Person person) {
        PersonEntity personEntity = testBusinessLogicService.processUpdate(id, person.getName());
        testBusinessLogicService.processUpdate(id,  person.getName());
        return new ResponseEntity<>(personEntity, HttpStatus.OK);
    }

    @DeleteMapping(path = {"/{id}"}, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<PersonEntity> deletePerson(@PathVariable String id) {
